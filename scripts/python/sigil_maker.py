...
def resolve_generation_llm(args: argparse.Namespace) -> GenerationLLMConfig:
    provider = str(getattr(args, "llm_provider", "sonnet") or "sonnet").strip().lower()
...
def emit_artifacts(spec: SpellSpec, args: argparse.Namespace) -> SpellCostBreakdown:
    ...
    # Spells are now consolidated in grimoires; individual pages no longer created
    ...
    def add_ollama_args(command_parser: argparse.ArgumentParser) -> None:
        command_parser.add_argument(
            ...
            help=(
                ...
                "Default is sonnet (Anthropic Claude). Choose local, ollama, sonnet, or openai."
            ),
        )
        command_parser.add_argument(
            "--ollama-model",
